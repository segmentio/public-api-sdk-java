...
public class UpdateDestinationV1Input {
  ...
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "An optional object that contains settings for the Destination based on the \"required\" and \"advanced\" settings present in the Destination metadata.  Config API note: equal to `config`.")
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!UpdateDestinationV1Input.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in UpdateDestinationV1Input is not found in the empty JSON string", UpdateDestinationV1Input.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!UpdateDestinationV1Input.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `UpdateDestinationV1Input` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!UpdateDestinationV1Input.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'UpdateDestinationV1Input' and its subtypes
       }
       ...
       return (TypeAdapter<T>) new TypeAdapter<UpdateDestinationV1Input>() {
           @Override
           public void write(JsonWriter out, UpdateDestinationV1Input value) throws IOException {
