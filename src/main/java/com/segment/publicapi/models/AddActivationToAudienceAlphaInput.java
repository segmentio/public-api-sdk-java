...
public class AddActivationToAudienceAlphaInput {
     ...
     * Sends an event when a profile or entity enters the audience. - **Audience Exited**: Sends an
     * event when a profile or entity exits the audience. - **Audience Membership Changed**: Sends
     * an event for both entries and exits. This does not apply to entities. - **Entity Added**:
     * Sends an event when an entity is added to the audience. - **Entity Removed**: Sends an event
     * when an entity is removed from the audience. - **Entity Values Changed**: Sends an event when
     * an entity&#39;s property values change. Restricted to freshness-enabled Destinations. - **All
