...
public class AddPermissionsToUserGroupV1Input {
    ...
    public boolean equals(Object o) {
        ...
        AddPermissionsToUserGroupV1Input addPermissionsToUserGroupV1Input =
                (AddPermissionsToUserGroupV1Input) o;
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!AddPermissionsToUserGroupV1Input.openapiRequiredFields
                    .isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(
                        String.format(
                                "The required field(s) %s in AddPermissionsToUserGroupV1Input is"
                                        + " not found in the empty JSON string",
                                AddPermissionsToUserGroupV1Input.openapiRequiredFields.toString()));
            }
        }
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "The field `%s` in the JSON string is not defined in the"
                                    + " `AddPermissionsToUserGroupV1Input` properties. JSON: %s",
                                entry.getKey(), jsonElement.toString()));
            }
        }

        // check to make sure all required properties/fields are present in the JSON string
        for (String requiredField : AddPermissionsToUserGroupV1Input.openapiRequiredFields) {
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!AddPermissionsToUserGroupV1Input.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'AddPermissionsToUserGroupV1Input' and
                // its subtypes
            }
            final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
            final TypeAdapter<AddPermissionsToUserGroupV1Input> thisAdapter =
                    gson.getDelegateAdapter(
                            this, TypeToken.get(AddPermissionsToUserGroupV1Input.class));

            return (TypeAdapter<T>)
                    new TypeAdapter<AddPermissionsToUserGroupV1Input>() {
                        @Override
                        public void write(JsonWriter out, AddPermissionsToUserGroupV1Input value)
                                throws IOException {
                        ...
                        @Override
                        public AddPermissionsToUserGroupV1Input read(JsonReader in)
                                throws IOException {
