...
public class WarehouseSyncOverrideV1 {
  ...
  */
  public static void validateJsonObject(JsonObject jsonObj) throws IOException {
      if (jsonObj == null) {
        if (!WarehouseSyncOverrideV1.openapiRequiredFields.isEmpty()) { // has required fields but JSON object is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in WarehouseSyncOverrideV1 is not found in the empty JSON string", WarehouseSyncOverrideV1.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Entry<String, JsonElement> entry : entries) {
        if (!WarehouseSyncOverrideV1.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `WarehouseSyncOverrideV1` properties. JSON: %s", entry.getKey(), jsonObj.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!WarehouseSyncOverrideV1.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'WarehouseSyncOverrideV1' and its subtypes
       }
       ...
       return (TypeAdapter<T>) new TypeAdapter<WarehouseSyncOverrideV1>() {
           @Override
           public void write(JsonWriter out, WarehouseSyncOverrideV1 value) throws IOException {
